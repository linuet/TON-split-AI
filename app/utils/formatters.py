...
def format_receipt(receipt: Receipt) -> str:
    ...
    for idx, item in enumerate(parsed.items, start=1):
        ...
        lines.append(f"{idx}. {item.normalized_name} — <b>{item.line_total}</b> {parsed.currency}{mark}")
...
def format_payment_requests(requests: list[PaymentRequest]) -> str:
    ...
    for req in requests:
        participant_name = req.participant.display_name if req.participant else f"Participant #{req.participant_id}"
        lines.append(
            f"• {participant_name}: <b>{req.amount_ton}</b> TON for <b>{req.amount_fiat}</b> fiat\n"
            f"  <a href=\"{req.payment_link}\">Open TON payment</a>"
        )
    return "\n".join(lines)
